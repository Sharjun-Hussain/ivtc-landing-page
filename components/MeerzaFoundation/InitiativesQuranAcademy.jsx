        ...
        <div className="flex flex-col lg:flex-row gap-16 lg:gap-24">
          ...
          <div className="lg:w-1/3">
            <ScrollReveal className="sticky top-32">
              <h2 id="quran-academy-heading" className="text-4xl md:text-6xl font-bold tracking-tight mb-8 leading-[1.1] text-slate-900 dark:text-white">
                Quranic <br /> Academy.
              </h2>
          ...
          <div className="lg:w-2/3 space-y-12">
            <div className="grid md:grid-cols-2 gap-8">
              {courses.map((course, idx) => (
                <ScrollReveal key={course.id} animationClass="animate-fade-in" options={{ delay: idx * 0.1 }}>
                  <div className="group relative p-10 rounded-[3rem] bg-white dark:bg-[#111] border border-slate-200 dark:border-white/5 hover:border-[#002147]/30 hover:shadow-2xl transition-all duration-500 h-full flex flex-col">
                    <div className={`inline-flex p-4 rounded-2xl mb-8 ${course.color} w-fit shadow-sm`}>
                      <BookOpen size={28} />
                    </div>
                    <h3 className="text-2xl font-bold text-slate-900 dark:text-white mb-4">{course.title}</h3>
                    <p className="text-sm text-slate-500 dark:text-slate-400 mb-8 leading-relaxed font-medium">{course.desc}</p>
            ...
            {/* Commitment Card */}
            <ScrollReveal>
              <div className="bg-slate-50 dark:bg-[#111] border border-slate-200 dark:border-white/5 rounded-[3rem] p-12 md:p-16 shadow-xl">
                <div className="grid md:grid-cols-2 gap-12 items-center">
                  <div>
                     <h3 className="text-[2.5rem] font-bold text-slate-900 dark:text-white mb-8 tracking-tight leading-tight">
                     ...
                     <ul className="space-y-4">
                       {rules.map((item, i) => (
                         <li key={i} className="flex items-start gap-4 text-sm font-medium text-slate-600 dark:text-slate-300">
                           <ShieldCheck size={20} className="text-[#002147] dark:text-blue-500 shrink-0 mt-0.5" /> {item}
                         </li>
                       ))}
                     </ul>
                  </div>
                  <div className="bg-white dark:bg-black p-10 rounded-[2.5rem] border border-slate-200 dark:border-white/10 shadow-lg text-center md:text-right">
                     <p className="text-xl font-bold text-slate-900 dark:text-white mb-2">Have Questions?</p>
                     <p className="text-slate-500 text-sm mb-8 font-medium">Contact our coordinator directly.</p>
                     <div className="inline-flex items-center gap-4 px-8 py-4 bg-[#002147] text-white rounded-2xl shadow-xl hover:scale-105 transition-transform cursor-pointer">
                        <MessageCircle size={22} className="text-green-400" />
                        <span className="font-bold tracking-widest text-sm">+94 71 541 9807</span>
                     </div>
                  </div>
                </div>
              </div>
