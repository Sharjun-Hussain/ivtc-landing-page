...
const HeroWithMegaMenu = () => {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      // Hero Entrance (Kept as requested)
      gsap.fromTo(
        videoRef.current,
  ...
  return (
    <>
      ...
      >
        ...
        >
          ...
          <Link href="/" className="flex items-center gap-2 z-[110]" onClick={() => setIsMenuOpen(false)}>
            ...
            >
              IVTC<span className={isScrolled ? "text-slate-900 dark:text-white" : "text-white"}>CAMPUS</span>
            </span>
          </Link>
              ...
              >
                ...
                {link.hasMega && activeMenu === link.name && (
                  <div className="absolute top-full -left-20 pt-6 cursor-default">
                    <div className="w-[600px] bg-white dark:bg-slate-900 rounded-[2.5rem] p-8 shadow-[0_25px_60px_-15px_rgba(0,0,0,0.3)] border border-slate-200 dark:border-white/10 text-left">
          ...
          <div className="flex flex-col gap-6">
            {navLinks.map((link, i) => (
              <Link
                ...
                className="text-4xl font-black tracking-tighter text-slate-900 dark:text-white flex items-center justify-between"
                style={{ transitionDelay: `${i * 50}ms` }}
              >
                {link.name}
                <ArrowRight className="text-slate-900 dark:text-white" size={28} />
              </Link>
            ))}
          </div>
      ...
      <section
        ref={containerRef}
        className="relative w-full flex justify-center bg-white dark:bg-[#0a0a0a] p-4 md:p-6 min-h-screen transition-colors duration-500"
      >
        <div className="relative w-full max-w-[1600px] aspect-16/10 md:aspect-video lg:max-h-[850px] overflow-hidden rounded-[2.5rem] md:rounded-[4rem] shadow-2xl bg-black">
          {/* --- VIDEO & HERO --- */}
          <video
            ref={videoRef}
            autoPlay
          ...
          <div className="relative z-10 flex flex-col justify-center h-full px-10 md:px-24">
            ...
            <h1 className="hero-text text-6xl md:text-[7.5rem] font-black text-white leading-[0.85] tracking-tighter max-w-4xl">
              Empower Your <br />
              <span className="text-white/90">Digital</span> Future.
            </h1>
            <div className="hero-text mt-12">
              <button className="px-12 py-5 bg-white text-slate-900 text-[11px] font-black uppercase tracking-widest rounded-full">
                Start Learning
              </button>
            </div>
          </div>
