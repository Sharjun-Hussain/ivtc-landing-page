...
// 1. MOVED OUTSIDE: Static data doesn't need to be recreated on every render
const footerLinks = [
  {
    title: "Programs",
    links: [
...
const Footer = () => {
  ...
  // 2. MEMOIZED FUNCTION: Prevents recreation on re-renders
  ...
  return (
    <footer className="relative bg-[#0a0a0a] text-white pt-24 pb-8 px-6 md:px-12 border-t border-white/5 overflow-hidden">
      ...
      {/* 3. PERFORMANCE FIX: Added 'hidden md:block' to disable the heavy CSS blur on mobile */}
            ...
            <div className="flex flex-wrap gap-3">
              {[Facebook, Twitter, Instagram, Linkedin, Github].map((Icon, i) => (
                <a
                  key={i}
                  href="#"
                  // 4. PERFORMANCE FIX: Changed transition-all to transition-colors
                  ...
                  {col.links.map((link, j) => (
                    <li key={j}>
                      ...
                      >
                        ...
                        {/* Changed transition-all to transition-opacity and transform */}
                        <ArrowUpRight
                          size={14}
