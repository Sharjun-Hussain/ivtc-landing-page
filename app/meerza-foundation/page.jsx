...
const InitiativesPage = () => {
  ...
  const [formData, setFormData] = useState({
    firstName: "", lastName: "", dob: "", address: "", district: "", phone: "", supportType: ""
  });
    ...
    >
      ...
      <section className="relative pt-32 pb-16 bg-slate-50 dark:bg-[#0d0d0d] border-b border-slate-200 dark:border-white/5">
        <div className="max-w-4xl mx-auto px-6 text-center">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white dark:bg-white/5 border border-amber-200 dark:border-amber-500/20 text-amber-600 text-[10px] font-black uppercase tracking-[0.2em] mb-6 shadow-sm">
            ...
            {[
              ...
              <button
                ...
                className={cn(
                  ...
                  activeTab === tab.id
                    ? "bg-slate-900 dark:bg-amber-600 text-white shadow-lg"
          ...
          <div className="tab-content grid md:grid-cols-2 gap-12 items-start">
            <div className="space-y-6">
              ...
              <div className="grid gap-4">
                {ACADEMIC_OFFERS.map((offer) => (
                  <div key={offer.id} className="p-6 rounded-2xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 hover:border-amber-500/30 transition-colors">
                    <div className="flex items-start gap-4">
                      <div className="w-12 h-12 rounded-xl bg-white dark:bg-white/5 flex items-center justify-center text-amber-600 shadow-sm">
                        <offer.icon size={24} />
                      </div>
                      <div>
                        ...
                        <button 
                          ...
                          className="text-xs font-bold uppercase tracking-widest text-amber-600 flex items-center gap-2 hover:gap-3 transition-all"
                        >
            ...
            <div ref={formRef} className="bg-white dark:bg-[#111] p-8 rounded-3xl border border-slate-200 dark:border-white/5 shadow-2xl">
              <h3 className="text-xl font-bold text-slate-900 dark:text-white mb-6 flex items-center gap-2">
                <FileText size={20} className="text-amber-600" /> Application Form
              </h3>
              <form className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <label className="text-[10px] uppercase font-bold text-slate-500">First Name</label>
                    <input className="w-full bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-lg p-3 text-sm focus:ring-2 focus:ring-amber-500" placeholder="John" />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[10px] uppercase font-bold text-slate-500">Last Name</label>
                    <input className="w-full bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-lg p-3 text-sm focus:ring-2 focus:ring-amber-500" placeholder="Doe" />
                  </div>
                </div>
                
                <div className="space-y-1">
                  <label className="text-[10px] uppercase font-bold text-slate-500">Support Needed</label>
                  <select className="w-full bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-lg p-3 text-sm focus:ring-2 focus:ring-amber-500">
                    <option>Select Option...</option>
                    <option>Free Books & Stationery</option>
                    <option>Free Quran Request</option>
                  </select>
                </div>

                <div className="space-y-1">
                    <label className="text-[10px] uppercase font-bold text-slate-500">Address</label>
                    <input className="w-full bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-lg p-3 text-sm focus:ring-2 focus:ring-amber-500" placeholder="Your permanent address" />
                </div>

                <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1">
                        <label className="text-[10px] uppercase font-bold text-slate-500">District</label>
                        <input className="w-full bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-lg p-3 text-sm focus:ring-2 focus:ring-amber-500" placeholder="Colombo" />
                    </div>
                    <div className="space-y-1">
                        <label className="text-[10px] uppercase font-bold text-slate-500">WhatsApp</label>
                        <input className="w-full bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-lg p-3 text-sm focus:ring-2 focus:ring-amber-500" placeholder="+94 7..." />
                    </div>
                </div>

                <button className="w-full py-4 rounded-xl bg-slate-900 dark:bg-amber-600 text-white font-bold text-sm uppercase tracking-widest hover:shadow-lg transition-all mt-4">
                  Submit Application
                </button>
              </form>
            </div>
        ...
        {activeTab === "academy" && (
          <div className="tab-content">
            ...
            <div className="grid md:grid-cols-3 gap-6 mb-16">
              {COURSES.map((course) => (
                <div key={course.id} className="group relative p-8 rounded-[2rem] bg-white dark:bg-[#111] border border-slate-200 dark:border-white/5 hover:border-amber-500/30 hover:shadow-xl transition-all">
                  ...
                  <button className="w-full py-3 rounded-xl bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 text-slate-900 dark:text-white font-bold text-xs uppercase tracking-widest group-hover:bg-amber-600 group-hover:text-white group-hover:border-amber-600 transition-all">
                    Register Now
                  </button>
                </div>
              ))}
            ...
            {/* CONDITIONS / INFO */}
            <div className="bg-amber-50 dark:bg-amber-900/10 border border-amber-100 dark:border-amber-500/20 rounded-2xl p-8 md:p-12">
                   ...
                   <ul className="space-y-3">
                     {[
                       ...
                       <li key={i} className="flex items-start gap-3 text-sm text-slate-600 dark:text-slate-300">
                         <ShieldCheck size={18} className="text-amber-600 shrink-0" /> {item}
                       </li>
                     ))}
                   </ul>
                </div>
        ...
        {activeTab === "library" && (
          <div className="tab-content">
            <div className="flex flex-col md:flex-row justify-between items-end mb-10 gap-4">
              ...
              <button className="text-xs font-bold uppercase tracking-widest text-amber-600 border-b border-amber-600 pb-1 hover:text-amber-700">
                View All Categories
              </button>
            </div>
                ...
                <div key={i} className="group bg-white dark:bg-[#111] p-6 rounded-2xl border border-slate-200 dark:border-white/5 hover:shadow-xl transition-all">
                  <div className="aspect-[3/4] bg-slate-100 dark:bg-black rounded-xl mb-6 overflow-hidden relative">
                    ...
                    <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                        <button className="px-6 py-2 bg-amber-600 text-white rounded-full font-bold text-xs uppercase tracking-widest transform translate-y-4 group-hover:translate-y-0 transition-all">
                            Preview
                        </button>
                    </div>
                  </div>
